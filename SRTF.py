...
def SRTF(process_list,cst):
    ...
    step=1
    #assume in this algorithm check every 1 sec
    while complete != num :
        ...
        if len(process_queue) > 0:
            ...
            if process_queue[0].remaining > step:
                process_queue[0].remaining -= step
                Current_Time += step
            else:
                Current_Time += process_queue[0].remaining
                process_queue[0].remaining = 0
                f_t[process_queue[0].pid] = Current_Time
                Current_Process[i-1].finish = Current_Time
                complete += 1
...
#        Current_Time+=step
    ...
    for i in range (0,len(process_list)) :
        ...
        process_list[i].wait = process_list[i].tat - process_list[i].running
